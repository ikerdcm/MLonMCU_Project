...
| `int4` | INT4 | **MAX78000 only** (ai8x QAT) ᵖ | — *(U5/X-CUBE-AI has no INT4 path — verified)* |
...
Model column: **Coral, U5, and MAX `fp32-cpu` (v0) all run the identical DS-CNN-L** (depthwise-separable, 3.94M MACs); **only MAX's `int8-accel` (v1) runs a different model** — the ai8x conv1d variant `kws-ds-cnn-l-kws12` (1.34M MACs) ᵏ, because the CNN accelerator doesn't run the depthwise-separable graph.
...
| **MAX78000** ᵏ | `fp32-cpu` | `ds_cnn_l` (**shared**, float CPU) | offline | 2048.5 | 2048.5 | 101639 | 170.1 | 68.5 | 92.4 | ✱ | ✅ prof + pwr | `MAX78000/…_v0/offline/` |
...
- **ᵏ ⚠️ MAX has TWO different models — check the row (verified 2026-06-08).** The **`fp32-cpu` (v0) row IS the shared DS-CNN-L**: a hand-coded float32 port (`ds_cnn_inference.c` + `ds_cnn_weights.h`, input 49×10, `CONV0 → DW(groups=64)+PW 1×1 ×6 → AvgPool → Dense 64→12`, ≈3.94M MACs) run on the Cortex-M4 with **no accelerator, no CMSIS-NN** — identical architecture to Coral/U5 v0 (the 2048 ms is precisely because it's naive float). The **`int8-accel` (v1) rows run a DIFFERENT model**: the MAX78000 CNN accelerator doesn't run the depthwise-separable graph, so it deploys the ai8x `kws-ds-cnn-l-kws12` (`ai85kwsmfccnet`) — **6× standard `conv1d`, ~1.34M MACs** (per `cnn.c` layer comments: conv1d k5,k5,k3,k3,k1 + linear), **not** depthwise-separable. So the MAX accelerator's **0.16 ms / energy are NOT a same-model comparison** (≈3× fewer MACs, different conv structure), and its accuracy is its own ai8x QAT number (the 92% in those cells is the TFLite value, to be replaced by ai8x eval). **Same-model cross-board comparison = the `fp32-cpu` (v0) row** (Coral = U5 = MAX-v0 = DS-CNN-L); treat the MAX accelerator as "best KWS this HW can do".
...
- **ᵖ INT4 investigation (X-CUBE-AI 10.2.0 on the Mac, `stedgeai analyze`, 2026-06-07).** With the toolchain in hand (ST Edge AI Core 2.2.0 + U5 on ST-Link) I verified that **there is no INT4 deployment path for DS-CNN on STM32U5**: (1) `stedgeai --quantize` only emits **int8/uint8** (`--input/output-data-type float32|int8|uint8`) — no sub-int8 quantizer; (2) `--compression {low|medium|high}` is **codebook (k-means) weight compression that targets large fully-connected matrices** — on this conv-dominated DS-CNN it is essentially a no-op: float weights **128,816 B → 126,192 B at `high` (−2 %)**, nowhere near the advertised ×4/×8, and `analyze` reports no per-layer compression. So the earlier "`--compression 4bit` would halve weights again" assumption was wrong. **The real 4-bit axis belongs to MAX78000**, whose ai8x accelerator does native per-layer 1/2/4/8-bit QAT weights — i.e. the 32→8→4 bit-width story is a **cross-MCU** result (32: Coral/U5 CPU · 8: Coral TPU + U5 CMSIS-NN · 4: MAX78000), *not* something U5 can deliver. Confirmed-faithful U5 int8 memory: weights 33.9 KiB, act 17.8 KiB, MACC 3.94 M.
...
- **Memory 32→8 bit (U5, `stedgeai analyze` verified 2026-06-07):** weights **125.8 KiB (fp32 onnx) → 33.9 KiB (int8)** = **3.7×** (−73.0 %); activations 58.3 KiB fp32 → 17.8 KiB int8; MACC 3.94 M. **No INT4 on U5** ᵖ — X-CUBE-AI 10.2.0 has no sub-int8 quantizer (`--quantize` is int8/uint8-only) and its `--compression high` codebook is a **no-op on DS-CNN** (128,816 → 126,192 B = **−2 %**, it targets large FC matrices; DS-CNN is conv-dominated). The genuine 4-bit axis lives on **MAX78000 (ai8x QAT, native 1/2/4/8-bit weights)**, not U5.
