#!/usr/bin/env bash
# Build and flash the kws_bench_cpu app (50-inference offline benchmark, FP32 on
# the M7 CPU — config: fp32-cpu, NO Edge TPU). Counterpart of
# build_and_flash_bench.sh; uses models/ds_cnn_l_float.tflite.
#
# Usage:  ./scripts/build_and_flash_bench_cpu.sh [path/to/coralmicro]
#
...
cmake --build "$BUILD_DIR" -j"$JOBS" --target elf_loader flashloader kws_bench_cpu
echo ""

echo "=== Flash kws_bench_cpu to Coral Dev Board Micro ==="
DYLD_LIBRARY_PATH=/opt/homebrew/lib${DYLD_LIBRARY_PATH:+:$DYLD_LIBRARY_PATH} \
"$VENV_PYTHON" "$CORALMICRO_ROOT/scripts/flashtool.py" \
    --build_dir "$BUILD_DIR" \
    --elf_path "$BUILD_DIR/kws_apps/kws_bench_cpu/kws_bench_cpu"
echo ""
echo "Flash complete. Connect serial terminal:"
echo "  screen \$(ls /dev/tty.usbmodem* 2>/dev/null | head -1) 115200"
echo ""
echo "Output: 50 BENCH CSV lines then summary stats (version=ds_cnn_l_cpu_fp32)."
