/**
  ******************************************************************************
  * @file    network.c
  * @author  AST Embedded Analytics Research Platform
  * @date    2026-05-10T11:12:33+0200
  * @brief   AI Tool Automatic Code Generator for Embedded NN computing
  ******************************************************************************
  * @attention
  *
  * Copyright (c) 2026 STMicroelectronics.
  * All rights reserved.
  *
  * This software is licensed under terms that can be found in the LICENSE file
  * in the root directory of this software component.
  * If no LICENSE file comes with this software, it is provided AS-IS.
  ******************************************************************************
  */


#include "network.h"
#include "network_data.h"

#include "ai_platform.h"
#include "ai_platform_interface.h"
#include "ai_math_helpers.h"

#include "core_common.h"
#include "core_convert.h"

#include "layers.h"



#undef AI_NET_OBJ_INSTANCE
#define AI_NET_OBJ_INSTANCE g_network
 
#undef AI_NETWORK_MODEL_SIGNATURE
#define AI_NETWORK_MODEL_SIGNATURE     "0x505bcfd5105ebe5078e1ed806bd95f5b"

#ifndef AI_TOOLS_REVISION_ID
#define AI_TOOLS_REVISION_ID     ""
#endif

#undef AI_TOOLS_DATE_TIME
#define AI_TOOLS_DATE_TIME   "2026-05-10T11:12:33+0200"

#undef AI_TOOLS_COMPILE_TIME
#define AI_TOOLS_COMPILE_TIME    __DATE__ " " __TIME__

#undef AI_NETWORK_N_BATCHES
#define AI_NETWORK_N_BATCHES         (1)

static ai_ptr g_network_activations_map[1] = AI_C_ARRAY_INIT;
static ai_ptr g_network_weights_map[1] = AI_C_ARRAY_INIT;



/**  Array declarations section  **********************************************/
/* Array#0 */
AI_ARRAY_OBJ_DECLARE(
  kws_input_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
  NULL, NULL, 16384, AI_STATIC)

/* Array#1 */
AI_ARRAY_OBJ_DECLARE(
  kws_input_Transpose_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 16384, AI_STATIC)

/* Array#2 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 11776, AI_STATIC)

/* Array#3 */
AI_ARRAY_OBJ_DECLARE(
  _relu_Relu_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 11776, AI_STATIC)

/* Array#4 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5544, AI_STATIC)

/* Array#5 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 3780, AI_STATIC)

/* Array#6 */
AI_ARRAY_OBJ_DECLARE(
  _relu_2_Relu_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 3780, AI_STATIC)

/* Array#7 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1320, AI_STATIC)

/* Array#8 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1800, AI_STATIC)

/* Array#9 */
AI_ARRAY_OBJ_DECLARE(
  _relu_4_Relu_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1800, AI_STATIC)

/* Array#10 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 1232, AI_STATIC)

/* Array#11 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 644, AI_STATIC)

/* Array#12 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 240, AI_STATIC)

/* Array#13 */
AI_ARRAY_OBJ_DECLARE(
  _relu_7_Relu_output_0_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 240, AI_STATIC)

/* Array#14 */
AI_ARRAY_OBJ_DECLARE(
  _Reshape_output_0_to_chlast_output_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 240, AI_STATIC)

/* Array#15 */
AI_ARRAY_OBJ_DECLARE(
  kws_logits_output_array, AI_ARRAY_FORMAT_FLOAT|AI_FMT_FLAG_IS_IO,
  NULL, NULL, 21, AI_STATIC)

/* Array#16 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 11776, AI_STATIC)

/* Array#17 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 92, AI_STATIC)

/* Array#18 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 24288, AI_STATIC)

/* Array#19 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 88, AI_STATIC)

/* Array#20 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 15840, AI_STATIC)

/* Array#21 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 60, AI_STATIC)

/* Array#22 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 7920, AI_STATIC)

/* Array#23 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 44, AI_STATIC)

/* Array#24 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 7920, AI_STATIC)

/* Array#25 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 60, AI_STATIC)

/* Array#26 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 15840, AI_STATIC)

/* Array#27 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 88, AI_STATIC)

/* Array#28 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 24288, AI_STATIC)

/* Array#29 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 92, AI_STATIC)

/* Array#30 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 33120, AI_STATIC)

/* Array#31 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 60, AI_STATIC)

/* Array#32 */
AI_ARRAY_OBJ_DECLARE(
  kws_logits_weights_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 5040, AI_STATIC)

/* Array#33 */
AI_ARRAY_OBJ_DECLARE(
  kws_logits_bias_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 21, AI_STATIC)

/* Array#34 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 128, AI_STATIC)

/* Array#35 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 276, AI_STATIC)

/* Array#36 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_scratch1_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 176, AI_STATIC)

/* Array#37 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 264, AI_STATIC)

/* Array#38 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 180, AI_STATIC)

/* Array#39 */
AI_ARRAY_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_scratch1_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 88, AI_STATIC)

/* Array#40 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 132, AI_STATIC)

/* Array#41 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 180, AI_STATIC)

/* Array#42 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_scratch1_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 176, AI_STATIC)

/* Array#43 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 264, AI_STATIC)

/* Array#44 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_scratch1_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 184, AI_STATIC)

/* Array#45 */
AI_ARRAY_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_scratch0_array, AI_ARRAY_FORMAT_FLOAT,
  NULL, NULL, 552, AI_STATIC)

/**  Tensor declarations section  *********************************************/
/* Tensor #0 */
AI_TENSOR_OBJ_DECLARE(
  _Reshape_output_0_to_chlast_output, AI_STATIC,
  0, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 4, 60), AI_STRIDE_INIT(4, 4, 4, 4, 16),
  1, &_Reshape_output_0_to_chlast_output_array, NULL)

/* Tensor #1 */
AI_TENSOR_OBJ_DECLARE(
  _Reshape_output_0_to_chlast_output0, AI_STATIC,
  1, 0x0,
  AI_SHAPE_INIT(4, 1, 240, 1, 1), AI_STRIDE_INIT(4, 4, 4, 960, 960),
  1, &_Reshape_output_0_to_chlast_output_array, NULL)

/* Tensor #2 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_bias, AI_STATIC,
  2, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 1), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_kws_conv1_Conv_output_0_bias_array, NULL)

/* Tensor #3 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_output, AI_STATIC,
  3, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 30), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_kws_conv1_Conv_output_0_output_array, NULL)

/* Tensor #4 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_scratch0, AI_STATIC,
  4, 0x0,
  AI_SHAPE_INIT(4, 1, 44, 1, 3), AI_STRIDE_INIT(4, 4, 4, 176, 176),
  1, &_kws_conv1_Conv_output_0_scratch0_array, NULL)

/* Tensor #5 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_weights, AI_STATIC,
  5, 0x0,
  AI_SHAPE_INIT(4, 44, 1, 3, 60), AI_STRIDE_INIT(4, 4, 176, 10560, 10560),
  1, &_kws_conv1_Conv_output_0_weights_array, NULL)

/* Tensor #6 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_bias, AI_STATIC,
  6, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 1), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_kws_conv2_Conv_output_0_bias_array, NULL)

/* Tensor #7 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_output, AI_STATIC,
  7, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 14), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_kws_conv2_Conv_output_0_output_array, NULL)

/* Tensor #8 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_scratch0, AI_STATIC,
  8, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 3), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_kws_conv2_Conv_output_0_scratch0_array, NULL)

/* Tensor #9 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_scratch1, AI_STATIC,
  9, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 2), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_kws_conv2_Conv_output_0_scratch1_array, NULL)

/* Tensor #10 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_weights, AI_STATIC,
  10, 0x0,
  AI_SHAPE_INIT(4, 60, 1, 3, 88), AI_STRIDE_INIT(4, 4, 240, 21120, 21120),
  1, &_kws_conv2_Conv_output_0_weights_array, NULL)

/* Tensor #11 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_bias, AI_STATIC,
  11, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 1), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_kws_conv3_Conv_output_0_bias_array, NULL)

/* Tensor #12 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_output, AI_STATIC,
  12, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 7), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_kws_conv3_Conv_output_0_output_array, NULL)

/* Tensor #13 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_scratch0, AI_STATIC,
  13, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 3), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_kws_conv3_Conv_output_0_scratch0_array, NULL)

/* Tensor #14 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_scratch1, AI_STATIC,
  14, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 2), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_kws_conv3_Conv_output_0_scratch1_array, NULL)

/* Tensor #15 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_weights, AI_STATIC,
  15, 0x0,
  AI_SHAPE_INIT(4, 88, 1, 3, 92), AI_STRIDE_INIT(4, 4, 352, 32384, 32384),
  1, &_kws_conv3_Conv_output_0_weights_array, NULL)

/* Tensor #16 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_bias, AI_STATIC,
  16, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 1), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_kws_conv4_Conv_output_0_bias_array, NULL)

/* Tensor #17 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_output, AI_STATIC,
  17, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 4), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_kws_conv4_Conv_output_0_output_array, NULL)

/* Tensor #18 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_scratch0, AI_STATIC,
  18, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 6), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_kws_conv4_Conv_output_0_scratch0_array, NULL)

/* Tensor #19 */
AI_TENSOR_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_weights, AI_STATIC,
  19, 0x0,
  AI_SHAPE_INIT(4, 92, 1, 6, 60), AI_STRIDE_INIT(4, 4, 368, 22080, 22080),
  1, &_kws_conv4_Conv_output_0_weights_array, NULL)

/* Tensor #20 */
AI_TENSOR_OBJ_DECLARE(
  _relu_2_Relu_output_0_output, AI_STATIC,
  20, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 63), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_relu_2_Relu_output_0_output_array, NULL)

/* Tensor #21 */
AI_TENSOR_OBJ_DECLARE(
  _relu_4_Relu_output_0_output, AI_STATIC,
  21, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 30), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_relu_4_Relu_output_0_output_array, NULL)

/* Tensor #22 */
AI_TENSOR_OBJ_DECLARE(
  _relu_7_Relu_output_0_output, AI_STATIC,
  22, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 4), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_relu_7_Relu_output_0_output_array, NULL)

/* Tensor #23 */
AI_TENSOR_OBJ_DECLARE(
  _relu_Relu_output_0_output, AI_STATIC,
  23, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 128), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_relu_Relu_output_0_output_array, NULL)

/* Tensor #24 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_bias, AI_STATIC,
  24, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 1), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_voice_conv1_Conv_output_0_bias_array, NULL)

/* Tensor #25 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_output, AI_STATIC,
  25, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 128), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_voice_conv1_Conv_output_0_output_array, NULL)

/* Tensor #26 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_scratch0, AI_STATIC,
  26, 0x0,
  AI_SHAPE_INIT(4, 1, 128, 1, 1), AI_STRIDE_INIT(4, 4, 4, 512, 512),
  1, &_voice_conv1_Conv_output_0_scratch0_array, NULL)

/* Tensor #27 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_weights, AI_STATIC,
  27, 0x0,
  AI_SHAPE_INIT(4, 128, 1, 1, 92), AI_STRIDE_INIT(4, 4, 512, 47104, 47104),
  1, &_voice_conv1_Conv_output_0_weights_array, NULL)

/* Tensor #28 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_bias, AI_STATIC,
  28, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 1), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_voice_conv2_Conv_output_0_bias_array, NULL)

/* Tensor #29 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_output, AI_STATIC,
  29, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 63), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_voice_conv2_Conv_output_0_output_array, NULL)

/* Tensor #30 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_scratch0, AI_STATIC,
  30, 0x0,
  AI_SHAPE_INIT(4, 1, 92, 1, 3), AI_STRIDE_INIT(4, 4, 4, 368, 368),
  1, &_voice_conv2_Conv_output_0_scratch0_array, NULL)

/* Tensor #31 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_scratch1, AI_STATIC,
  31, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 2), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_voice_conv2_Conv_output_0_scratch1_array, NULL)

/* Tensor #32 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_weights, AI_STATIC,
  32, 0x0,
  AI_SHAPE_INIT(4, 92, 1, 3, 88), AI_STRIDE_INIT(4, 4, 368, 32384, 32384),
  1, &_voice_conv2_Conv_output_0_weights_array, NULL)

/* Tensor #33 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_bias, AI_STATIC,
  33, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 1), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_voice_conv3_Conv_output_0_bias_array, NULL)

/* Tensor #34 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_output, AI_STATIC,
  34, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 63), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_voice_conv3_Conv_output_0_output_array, NULL)

/* Tensor #35 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_scratch0, AI_STATIC,
  35, 0x0,
  AI_SHAPE_INIT(4, 1, 88, 1, 3), AI_STRIDE_INIT(4, 4, 4, 352, 352),
  1, &_voice_conv3_Conv_output_0_scratch0_array, NULL)

/* Tensor #36 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_weights, AI_STATIC,
  36, 0x0,
  AI_SHAPE_INIT(4, 88, 1, 3, 60), AI_STRIDE_INIT(4, 4, 352, 21120, 21120),
  1, &_voice_conv3_Conv_output_0_weights_array, NULL)

/* Tensor #37 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_bias, AI_STATIC,
  37, 0x0,
  AI_SHAPE_INIT(4, 1, 44, 1, 1), AI_STRIDE_INIT(4, 4, 4, 176, 176),
  1, &_voice_conv4_Conv_output_0_bias_array, NULL)

/* Tensor #38 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_output, AI_STATIC,
  38, 0x0,
  AI_SHAPE_INIT(4, 1, 44, 1, 30), AI_STRIDE_INIT(4, 4, 4, 176, 176),
  1, &_voice_conv4_Conv_output_0_output_array, NULL)

/* Tensor #39 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_scratch0, AI_STATIC,
  39, 0x0,
  AI_SHAPE_INIT(4, 1, 60, 1, 3), AI_STRIDE_INIT(4, 4, 4, 240, 240),
  1, &_voice_conv4_Conv_output_0_scratch0_array, NULL)

/* Tensor #40 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_scratch1, AI_STATIC,
  40, 0x0,
  AI_SHAPE_INIT(4, 1, 44, 1, 2), AI_STRIDE_INIT(4, 4, 4, 176, 176),
  1, &_voice_conv4_Conv_output_0_scratch1_array, NULL)

/* Tensor #41 */
AI_TENSOR_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_weights, AI_STATIC,
  41, 0x0,
  AI_SHAPE_INIT(4, 60, 1, 3, 44), AI_STRIDE_INIT(4, 4, 240, 10560, 10560),
  1, &_voice_conv4_Conv_output_0_weights_array, NULL)

/* Tensor #42 */
AI_TENSOR_OBJ_DECLARE(
  kws_input_Transpose_output, AI_STATIC,
  42, 0x0,
  AI_SHAPE_INIT(4, 1, 128, 1, 128), AI_STRIDE_INIT(4, 4, 4, 512, 512),
  1, &kws_input_Transpose_output_array, NULL)

/* Tensor #43 */
AI_TENSOR_OBJ_DECLARE(
  kws_input_output, AI_STATIC,
  43, 0x0,
  AI_SHAPE_INIT(4, 1, 1, 128, 128), AI_STRIDE_INIT(4, 4, 4, 4, 512),
  1, &kws_input_output_array, NULL)

/* Tensor #44 */
AI_TENSOR_OBJ_DECLARE(
  kws_logits_bias, AI_STATIC,
  44, 0x0,
  AI_SHAPE_INIT(4, 1, 21, 1, 1), AI_STRIDE_INIT(4, 4, 4, 84, 84),
  1, &kws_logits_bias_array, NULL)

/* Tensor #45 */
AI_TENSOR_OBJ_DECLARE(
  kws_logits_output, AI_STATIC,
  45, 0x0,
  AI_SHAPE_INIT(4, 1, 21, 1, 1), AI_STRIDE_INIT(4, 4, 4, 84, 84),
  1, &kws_logits_output_array, NULL)

/* Tensor #46 */
AI_TENSOR_OBJ_DECLARE(
  kws_logits_weights, AI_STATIC,
  46, 0x0,
  AI_SHAPE_INIT(4, 240, 21, 1, 1), AI_STRIDE_INIT(4, 4, 960, 20160, 20160),
  1, &kws_logits_weights_array, NULL)



/**  Layer declarations section  **********************************************/


AI_TENSOR_CHAIN_OBJ_DECLARE(
  kws_logits_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_Reshape_output_0_to_chlast_output0),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &kws_logits_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &kws_logits_weights, &kws_logits_bias),
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  kws_logits_layer, 23,
  DENSE_TYPE, 0x0, NULL,
  dense, forward_dense,
  &kws_logits_chain,
  NULL, &kws_logits_layer, AI_STATIC, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _Reshape_output_0_to_chlast_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_7_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_Reshape_output_0_to_chlast_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  _Reshape_output_0_to_chlast_layer, 22,
  TRANSPOSE_TYPE, 0x0, NULL,
  transpose, forward_transpose,
  &_Reshape_output_0_to_chlast_chain,
  NULL, &kws_logits_layer, AI_STATIC, 
  .out_mapping = AI_SHAPE_INIT(6, AI_SHAPE_IN_CHANNEL, AI_SHAPE_WIDTH, AI_SHAPE_HEIGHT, AI_SHAPE_CHANNEL, AI_SHAPE_DEPTH, AI_SHAPE_EXTENSION), 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _relu_7_Relu_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv4_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_7_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  _relu_7_Relu_output_0_layer, 20,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &_relu_7_Relu_output_0_chain,
  NULL, &_Reshape_output_0_to_chlast_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv3_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv4_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_kws_conv4_Conv_output_0_weights, &_kws_conv4_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_kws_conv4_Conv_output_0_scratch0, NULL)
)

AI_LAYER_OBJ_DECLARE(
  _kws_conv4_Conv_output_0_layer, 19,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d_if32of32wf32,
  &_kws_conv4_Conv_output_0_chain,
  NULL, &_relu_7_Relu_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 1, 0, 1, 0), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_SAME, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv2_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv3_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_kws_conv3_Conv_output_0_weights, &_kws_conv3_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_kws_conv3_Conv_output_0_scratch0, &_kws_conv3_Conv_output_0_scratch1)
)

AI_LAYER_OBJ_DECLARE(
  _kws_conv3_Conv_output_0_layer, 18,
  OPTIMIZED_CONV2D_TYPE, 0x0, NULL,
  conv2d_nl_pool, forward_conv2d_if32of32wf32_nl_pool,
  &_kws_conv3_Conv_output_0_chain,
  NULL, &_kws_conv4_Conv_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = AI_HANDLE_PTR(forward_lite_nl_relu_if32of32), 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 1, 0, 1, 0), 
  .pool_size = AI_SHAPE_2D_INIT(1, 2), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_func = AI_HANDLE_PTR(pool_func_mp_array_f32), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_SAME, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_4_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv2_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_kws_conv2_Conv_output_0_weights, &_kws_conv2_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_kws_conv2_Conv_output_0_scratch0, &_kws_conv2_Conv_output_0_scratch1)
)

AI_LAYER_OBJ_DECLARE(
  _kws_conv2_Conv_output_0_layer, 15,
  OPTIMIZED_CONV2D_TYPE, 0x0, NULL,
  conv2d_nl_pool, forward_conv2d_if32of32wf32_nl_pool,
  &_kws_conv2_Conv_output_0_chain,
  NULL, &_kws_conv3_Conv_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = AI_HANDLE_PTR(forward_lite_nl_relu_if32of32), 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_size = AI_SHAPE_2D_INIT(1, 2), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_func = AI_HANDLE_PTR(pool_func_ap_array_f32), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _relu_4_Relu_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv1_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_4_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  _relu_4_Relu_output_0_layer, 12,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &_relu_4_Relu_output_0_chain,
  NULL, &_kws_conv2_Conv_output_0_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv4_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_kws_conv1_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_kws_conv1_Conv_output_0_weights, &_kws_conv1_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_kws_conv1_Conv_output_0_scratch0, NULL)
)

AI_LAYER_OBJ_DECLARE(
  _kws_conv1_Conv_output_0_layer, 11,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d_if32of32wf32,
  &_kws_conv1_Conv_output_0_chain,
  NULL, &_relu_4_Relu_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 1, 0, 1, 0), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_SAME, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_2_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv4_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_voice_conv4_Conv_output_0_weights, &_voice_conv4_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_voice_conv4_Conv_output_0_scratch0, &_voice_conv4_Conv_output_0_scratch1)
)

AI_LAYER_OBJ_DECLARE(
  _voice_conv4_Conv_output_0_layer, 10,
  OPTIMIZED_CONV2D_TYPE, 0x0, NULL,
  conv2d_nl_pool, forward_conv2d_if32of32wf32_nl_pool,
  &_voice_conv4_Conv_output_0_chain,
  NULL, &_kws_conv1_Conv_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = AI_HANDLE_PTR(forward_lite_nl_relu_if32of32), 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_size = AI_SHAPE_2D_INIT(1, 2), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_func = AI_HANDLE_PTR(pool_func_mp_array_f32), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _relu_2_Relu_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv3_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_2_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  _relu_2_Relu_output_0_layer, 7,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &_relu_2_Relu_output_0_chain,
  NULL, &_voice_conv4_Conv_output_0_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv2_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv3_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_voice_conv3_Conv_output_0_weights, &_voice_conv3_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_voice_conv3_Conv_output_0_scratch0, NULL)
)

AI_LAYER_OBJ_DECLARE(
  _voice_conv3_Conv_output_0_layer, 6,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d_if32of32wf32,
  &_voice_conv3_Conv_output_0_chain,
  NULL, &_relu_2_Relu_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 1, 0, 1, 0), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_SAME, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv2_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_voice_conv2_Conv_output_0_weights, &_voice_conv2_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_voice_conv2_Conv_output_0_scratch0, &_voice_conv2_Conv_output_0_scratch1)
)

AI_LAYER_OBJ_DECLARE(
  _voice_conv2_Conv_output_0_layer, 5,
  OPTIMIZED_CONV2D_TYPE, 0x0, NULL,
  conv2d_nl_pool, forward_conv2d_if32of32wf32_nl_pool,
  &_voice_conv2_Conv_output_0_chain,
  NULL, &_voice_conv3_Conv_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .nl_params = NULL, 
  .nl_func = AI_HANDLE_PTR(forward_lite_nl_relu_if32of32), 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_size = AI_SHAPE_2D_INIT(1, 2), 
  .pool_stride = AI_SHAPE_2D_INIT(1, 2), 
  .pool_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .pool_func = AI_HANDLE_PTR(pool_func_mp_array_f32), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _relu_Relu_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv1_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_relu_Relu_output_0_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  _relu_Relu_output_0_layer, 2,
  NL_TYPE, 0x0, NULL,
  nl, forward_relu,
  &_relu_Relu_output_0_chain,
  NULL, &_voice_conv2_Conv_output_0_layer, AI_STATIC, 
  .nl_params = NULL, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &kws_input_Transpose_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &_voice_conv1_Conv_output_0_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 3, &_voice_conv1_Conv_output_0_weights, &_voice_conv1_Conv_output_0_bias, NULL),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 2, &_voice_conv1_Conv_output_0_scratch0, NULL)
)

AI_LAYER_OBJ_DECLARE(
  _voice_conv1_Conv_output_0_layer, 1,
  CONV2D_TYPE, 0x0, NULL,
  conv2d, forward_conv2d_if32of32wf32,
  &_voice_conv1_Conv_output_0_chain,
  NULL, &_relu_Relu_output_0_layer, AI_STATIC, 
  .groups = 1, 
  .filter_stride = AI_SHAPE_2D_INIT(1, 1), 
  .dilation = AI_SHAPE_2D_INIT(1, 1), 
  .filter_pad = AI_SHAPE_INIT(4, 0, 0, 0, 0), 
  .in_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
  .out_ch_format = AI_LAYER_FORMAT_CHANNEL_LAST_VALID, 
)

AI_TENSOR_CHAIN_OBJ_DECLARE(
  kws_input_Transpose_chain, AI_STATIC_CONST, 4,
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &kws_input_output),
  AI_TENSOR_LIST_OBJ_INIT(AI_FLAG_NONE, 1, &kws_input_Transpose_output),
  AI_TENSOR_LIST_OBJ_EMPTY,
  AI_TENSOR_LIST_OBJ_EMPTY
)

AI_LAYER_OBJ_DECLARE(
  kws_input_Transpose_layer, 2,
  TRANSPOSE_TYPE, 0x0, NULL,
  transpose, forward_transpose,
  &kws_input_Transpose_chain,
  NULL, &_voice_conv1_Conv_output_0_layer, AI_STATIC, 
  .out_mapping = AI_SHAPE_INIT(6, AI_SHAPE_IN_CHANNEL, AI_SHAPE_HEIGHT, AI_SHAPE_CHANNEL, AI_SHAPE_WIDTH, AI_SHAPE_DEPTH, AI_SHAPE_EXTENSION), 
)


#if (AI_TOOLS_API_VERSION < AI_TOOLS_API_VERSION_1_5)

AI_NETWORK_OBJ_DECLARE(
  AI_NET_OBJ_INSTANCE, AI_STATIC,
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 586548, 1, 1),
    586548, NULL, NULL),
  AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
    AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 131072, 1, 1),
    131072, NULL, NULL),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_IN_NUM, &kws_input_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_OUT_NUM, &kws_logits_output),
  &kws_input_Transpose_layer, 0x1253a7ea, NULL)

#else

AI_NETWORK_OBJ_DECLARE(
  AI_NET_OBJ_INSTANCE, AI_STATIC,
  AI_BUFFER_ARRAY_OBJ_INIT_STATIC(
  	AI_FLAG_NONE, 1,
    AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
      AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 586548, 1, 1),
      586548, NULL, NULL)
  ),
  AI_BUFFER_ARRAY_OBJ_INIT_STATIC(
  	AI_FLAG_NONE, 1,
    AI_BUFFER_INIT(AI_FLAG_NONE,  AI_BUFFER_FORMAT_U8,
      AI_BUFFER_SHAPE_INIT(AI_SHAPE_BCWH, 4, 1, 131072, 1, 1),
      131072, NULL, NULL)
  ),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_IN_NUM, &kws_input_output),
  AI_TENSOR_LIST_IO_OBJ_INIT(AI_FLAG_NONE, AI_NETWORK_OUT_NUM, &kws_logits_output),
  &kws_input_Transpose_layer, 0x1253a7ea, NULL)

#endif	/*(AI_TOOLS_API_VERSION < AI_TOOLS_API_VERSION_1_5)*/



/******************************************************************************/
AI_DECLARE_STATIC
ai_bool network_configure_activations(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_activations_map(g_network_activations_map, 1, params)) {
    /* Updating activations (byte) offsets */
    
    kws_input_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    kws_input_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    kws_input_Transpose_output_array.data = AI_PTR(g_network_activations_map[0] + 65536);
    kws_input_Transpose_output_array.data_start = AI_PTR(g_network_activations_map[0] + 65536);
    _voice_conv1_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _voice_conv1_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _voice_conv1_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 512);
    _voice_conv1_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 512);
    _relu_Relu_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 47616);
    _relu_Relu_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 47616);
    _voice_conv2_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _voice_conv2_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _voice_conv2_Conv_output_0_scratch1_array.data = AI_PTR(g_network_activations_map[0] + 1104);
    _voice_conv2_Conv_output_0_scratch1_array.data_start = AI_PTR(g_network_activations_map[0] + 1104);
    _voice_conv2_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 1808);
    _voice_conv2_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 1808);
    _voice_conv3_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _voice_conv3_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _voice_conv3_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 23984);
    _voice_conv3_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 23984);
    _relu_2_Relu_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _relu_2_Relu_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _voice_conv4_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 15120);
    _voice_conv4_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 15120);
    _voice_conv4_Conv_output_0_scratch1_array.data = AI_PTR(g_network_activations_map[0] + 15840);
    _voice_conv4_Conv_output_0_scratch1_array.data_start = AI_PTR(g_network_activations_map[0] + 15840);
    _voice_conv4_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 16192);
    _voice_conv4_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 16192);
    _kws_conv1_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv1_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv1_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 528);
    _kws_conv1_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 528);
    _relu_4_Relu_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 7728);
    _relu_4_Relu_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 7728);
    _kws_conv2_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv2_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv2_Conv_output_0_scratch1_array.data = AI_PTR(g_network_activations_map[0] + 720);
    _kws_conv2_Conv_output_0_scratch1_array.data_start = AI_PTR(g_network_activations_map[0] + 720);
    _kws_conv2_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 1424);
    _kws_conv2_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 1424);
    _kws_conv3_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv3_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv3_Conv_output_0_scratch1_array.data = AI_PTR(g_network_activations_map[0] + 6352);
    _kws_conv3_Conv_output_0_scratch1_array.data_start = AI_PTR(g_network_activations_map[0] + 6352);
    _kws_conv3_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 7088);
    _kws_conv3_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 7088);
    _kws_conv4_Conv_output_0_scratch0_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv4_Conv_output_0_scratch0_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _kws_conv4_Conv_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 2208);
    _kws_conv4_Conv_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 2208);
    _relu_7_Relu_output_0_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    _relu_7_Relu_output_0_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    _Reshape_output_0_to_chlast_output_array.data = AI_PTR(g_network_activations_map[0] + 960);
    _Reshape_output_0_to_chlast_output_array.data_start = AI_PTR(g_network_activations_map[0] + 960);
    kws_logits_output_array.data = AI_PTR(g_network_activations_map[0] + 0);
    kws_logits_output_array.data_start = AI_PTR(g_network_activations_map[0] + 0);
    return true;
  }
  AI_ERROR_TRAP(net_ctx, INIT_FAILED, NETWORK_ACTIVATIONS);
  return false;
}




/******************************************************************************/
AI_DECLARE_STATIC
ai_bool network_configure_weights(
  ai_network* net_ctx, const ai_network_params* params)
{
  AI_ASSERT(net_ctx)

  if (ai_platform_get_weights_map(g_network_weights_map, 1, params)) {
    /* Updating weights (byte) offsets */
    
    _voice_conv1_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv1_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 0);
    _voice_conv1_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 0);
    _voice_conv1_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv1_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 47104);
    _voice_conv1_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 47104);
    _voice_conv2_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv2_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 47472);
    _voice_conv2_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 47472);
    _voice_conv2_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv2_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 144624);
    _voice_conv2_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 144624);
    _voice_conv3_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv3_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 144976);
    _voice_conv3_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 144976);
    _voice_conv3_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv3_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 208336);
    _voice_conv3_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 208336);
    _voice_conv4_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv4_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 208576);
    _voice_conv4_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 208576);
    _voice_conv4_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _voice_conv4_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 240256);
    _voice_conv4_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 240256);
    _kws_conv1_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv1_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 240432);
    _kws_conv1_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 240432);
    _kws_conv1_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv1_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 272112);
    _kws_conv1_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 272112);
    _kws_conv2_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv2_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 272352);
    _kws_conv2_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 272352);
    _kws_conv2_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv2_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 335712);
    _kws_conv2_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 335712);
    _kws_conv3_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv3_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 336064);
    _kws_conv3_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 336064);
    _kws_conv3_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv3_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 433216);
    _kws_conv3_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 433216);
    _kws_conv4_Conv_output_0_weights_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv4_Conv_output_0_weights_array.data = AI_PTR(g_network_weights_map[0] + 433584);
    _kws_conv4_Conv_output_0_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 433584);
    _kws_conv4_Conv_output_0_bias_array.format |= AI_FMT_FLAG_CONST;
    _kws_conv4_Conv_output_0_bias_array.data = AI_PTR(g_network_weights_map[0] + 566064);
    _kws_conv4_Conv_output_0_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 566064);
    kws_logits_weights_array.format |= AI_FMT_FLAG_CONST;
    kws_logits_weights_array.data = AI_PTR(g_network_weights_map[0] + 566304);
    kws_logits_weights_array.data_start = AI_PTR(g_network_weights_map[0] + 566304);
    kws_logits_bias_array.format |= AI_FMT_FLAG_CONST;
    kws_logits_bias_array.data = AI_PTR(g_network_weights_map[0] + 586464);
    kws_logits_bias_array.data_start = AI_PTR(g_network_weights_map[0] + 586464);
    return true;
  }
  AI_ERROR_TRAP(net_ctx, INIT_FAILED, NETWORK_WEIGHTS);
  return false;
}


/**  PUBLIC APIs SECTION  *****************************************************/



AI_DEPRECATED
AI_API_ENTRY
ai_bool ai_network_get_info(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);

  if (report && net_ctx)
  {
    ai_network_report r = {
      .model_name        = AI_NETWORK_MODEL_NAME,
      .model_signature   = AI_NETWORK_MODEL_SIGNATURE,
      .model_datetime    = AI_TOOLS_DATE_TIME,
      
      .compile_datetime  = AI_TOOLS_COMPILE_TIME,
      
      .runtime_revision  = ai_platform_runtime_get_revision(),
      .runtime_version   = ai_platform_runtime_get_version(),

      .tool_revision     = AI_TOOLS_REVISION_ID,
      .tool_version      = {AI_TOOLS_VERSION_MAJOR, AI_TOOLS_VERSION_MINOR,
                            AI_TOOLS_VERSION_MICRO, 0x0},
      .tool_api_version  = AI_STRUCT_INIT,

      .api_version            = ai_platform_api_get_version(),
      .interface_api_version  = ai_platform_interface_api_get_version(),
      
      .n_macc            = 7268845,
      .n_inputs          = 0,
      .inputs            = NULL,
      .n_outputs         = 0,
      .outputs           = NULL,
      .params            = AI_STRUCT_INIT,
      .activations       = AI_STRUCT_INIT,
      .n_nodes           = 0,
      .signature         = 0x1253a7ea,
    };

    if (!ai_platform_api_get_network_report(network, &r)) return false;

    *report = r;
    return true;
  }
  return false;
}



AI_API_ENTRY
ai_bool ai_network_get_report(
  ai_handle network, ai_network_report* report)
{
  ai_network* net_ctx = AI_NETWORK_ACQUIRE_CTX(network);

  if (report && net_ctx)
  {
    ai_network_report r = {
      .model_name        = AI_NETWORK_MODEL_NAME,
      .model_signature   = AI_NETWORK_MODEL_SIGNATURE,
      .model_datetime    = AI_TOOLS_DATE_TIME,
      
      .compile_datetime  = AI_TOOLS_COMPILE_TIME,
      
      .runtime_revision  = ai_platform_runtime_get_revision(),
      .runtime_version   = ai_platform_runtime_get_version(),

      .tool_revision     = AI_TOOLS_REVISION_ID,
      .tool_version      = {AI_TOOLS_VERSION_MAJOR, AI_TOOLS_VERSION_MINOR,
                            AI_TOOLS_VERSION_MICRO, 0x0},
      .tool_api_version  = AI_STRUCT_INIT,

      .api_version            = ai_platform_api_get_version(),
      .interface_api_version  = ai_platform_interface_api_get_version(),
      
      .n_macc            = 7268845,
      .n_inputs          = 0,
      .inputs            = NULL,
      .n_outputs         = 0,
      .outputs           = NULL,
      .map_signature     = AI_MAGIC_SIGNATURE,
      .map_weights       = AI_STRUCT_INIT,
      .map_activations   = AI_STRUCT_INIT,
      .n_nodes           = 0,
      .signature         = 0x1253a7ea,
    };

    if (!ai_platform_api_get_network_report(network, &r)) return false;

    *report = r;
    return true;
  }
  return false;
}


AI_API_ENTRY
ai_error ai_network_get_error(ai_handle network)
{
  return ai_platform_network_get_error(network);
}


AI_API_ENTRY
ai_error ai_network_create(
  ai_handle* network, const ai_buffer* network_config)
{
  return ai_platform_network_create(
    network, network_config, 
    AI_CONTEXT_OBJ(&AI_NET_OBJ_INSTANCE),
    AI_TOOLS_API_VERSION_MAJOR, AI_TOOLS_API_VERSION_MINOR, AI_TOOLS_API_VERSION_MICRO);
}


AI_API_ENTRY
ai_error ai_network_create_and_init(
  ai_handle* network, const ai_handle activations[], const ai_handle weights[])
{
  ai_error err;
  ai_network_params params;

  err = ai_network_create(network, AI_NETWORK_DATA_CONFIG);
  if (err.type != AI_ERROR_NONE) {
    return err;
  }
  
  if (ai_network_data_params_get(&params) != true) {
    err = ai_network_get_error(*network);
    return err;
  }
#if defined(AI_NETWORK_DATA_ACTIVATIONS_COUNT)
  /* set the addresses of the activations buffers */
  for (ai_u16 idx=0; activations && idx<params.map_activations.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_activations, idx, activations[idx]);
  }
#endif
#if defined(AI_NETWORK_DATA_WEIGHTS_COUNT)
  /* set the addresses of the weight buffers */
  for (ai_u16 idx=0; weights && idx<params.map_weights.size; idx++) {
    AI_BUFFER_ARRAY_ITEM_SET_ADDRESS(&params.map_weights, idx, weights[idx]);
  }
#endif
  if (ai_network_init(*network, &params) != true) {
    err = ai_network_get_error(*network);
  }
  return err;
}


AI_API_ENTRY
ai_buffer* ai_network_inputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    AI_NETWORK_OBJ(network)->magic = AI_MAGIC_CONTEXT_TOKEN;
  }
  return ai_platform_inputs_get(network, n_buffer);
}


AI_API_ENTRY
ai_buffer* ai_network_outputs_get(ai_handle network, ai_u16 *n_buffer)
{
  if (network == AI_HANDLE_NULL) {
    network = (ai_handle)&AI_NET_OBJ_INSTANCE;
    AI_NETWORK_OBJ(network)->magic = AI_MAGIC_CONTEXT_TOKEN;
  }
  return ai_platform_outputs_get(network, n_buffer);
}


AI_API_ENTRY
ai_handle ai_network_destroy(ai_handle network)
{
  return ai_platform_network_destroy(network);
}


AI_API_ENTRY
ai_bool ai_network_init(
  ai_handle network, const ai_network_params* params)
{
  ai_network* net_ctx = AI_NETWORK_OBJ(ai_platform_network_init(network, params));
  ai_bool ok = true;

  if (!net_ctx) return false;
  ok &= network_configure_weights(net_ctx, params);
  ok &= network_configure_activations(net_ctx, params);

  ok &= ai_platform_network_post_init(network);

  return ok;
}


AI_API_ENTRY
ai_i32 ai_network_run(
  ai_handle network, const ai_buffer* input, ai_buffer* output)
{
  return ai_platform_network_process(network, input, output);
}


AI_API_ENTRY
ai_i32 ai_network_forward(ai_handle network, const ai_buffer* input)
{
  return ai_platform_network_process(network, input, NULL);
}



#undef AI_NETWORK_MODEL_SIGNATURE
#undef AI_NET_OBJ_INSTANCE
#undef AI_TOOLS_DATE_TIME
#undef AI_TOOLS_COMPILE_TIME

