#!/usr/bin/env python3
"""
Offline measurement script for DS-CNN-L INT8 on GAP9 board.
...
MUST run inside the Deeploy Docker container after a successful build:
  cd /app/Deeploy/DeeployTest
  python deeployRunner_gap9.py -t Tests/Models/DSCNNL_INT8 -s board
  cd /app/Deeploy
  python tools/kws20_measure_gap9_ds_cnn_int8.py [--runs 20]

Outputs (saved to /app/gap9_measurements/offline_measurements/<timestamp>/):
  summary.json       -- statistics + metadata
  cycles_raw.csv     -- per-run cycles / latency / predicted class / logit
...
DEFAULT_CONFIG_PATH = Path(__file__).with_name("gap9_ds_cnn_int8_measure_config.json")
...
# Float format: "Predicted class: N (confidence F)"
RE_PRED     = re.compile(_SH + r'Predicted\s+class:\s*(\d+)\s*'
                                r'\(confidence\s+([0-9.eE+\-]+)\)',     re.I)
# Integer/quantized format: "Predicted class: N (logit M)"
RE_PRED_INT = re.compile(_SH + r'Predicted\s+class:\s*(\d+)\s*'
                                r'\(logit\s+(-?\d+)\)',                  re.I)
RE_L2MEM    = re.compile(
    r'(L2[\w_]*):\s+([\d.]+)\s+(KB|B)\s+([\d.]+)\s+(KB|MB)\s+([\d.]+)%')

GAP9_SDK_DEFAULT = "/app/install/gap9-sdk"
BINARY_SUBPATH   = "DeeployTest/TEST_GAP9/build_master/DSCNNL_INT8"
WORKDIR_SUBPATH  = "DeeployTest/TEST_GAP9/build_master/board_workdir_int8"
...
def run_inference(gapy_cmd, env, run_idx, log_dir):
    ...
    Returns (cycles, errors_n, tested_n, pred_class, pred_logit) or None on timeout.
    pred_logit is the raw integer logit value for the predicted class.
    """
    ...
    errors_n = tested_n = pred_class = pred_logit = None
    ...
    # Try integer format first (logit), then float format (confidence)
    m = RE_PRED_INT.search(raw)
    if m:
        pred_class = int(m.group(1))
        pred_logit = int(m.group(2))
    else:
        m = RE_PRED.search(raw)
        if m:
            pred_class = int(m.group(1))
            pred_logit = int(float(m.group(2)))  # treat confidence as logit fallback

    return cycles, errors_n, tested_n, pred_class, pred_logit
...
def main():
    ...
    ap = argparse.ArgumentParser(
        description="Measure DS-CNN-L INT8 inference on GAP9 board (runs inside Docker container)")
    ap.add_argument("--config",          default=str(config_path))
    ap.add_argument("--runs",            type=int,   default=cfg.get("runs", 20),
    ...
    ap.add_argument("--variant-name",    default=cfg.get("variant_name", "ds_cnn_l_int8"))
    ...
    if not binary_path.exists():
        ...
        print("  python deeployRunner_gap9.py -t Tests/Models/DSCNNL_INT8 -s board")
        sys.exit(1)
    ...
    print(f"[GAP9 Measure INT8] Output:  {out_dir}")
    print(f"[GAP9 Measure INT8] Binary:  {binary_path}")
    print(f"[GAP9 Measure INT8] Runs:    {args.runs}")
    print(f"[GAP9 Measure INT8] Clock:   {args.clock_hz/1e6:.0f} MHz")

    # ── ELF size ───────────────────────────────────────────────────────────────
    elf_size = run_size_on_elf(binary_path, sdk_home)
    if elf_size:
        print(f"[GAP9 Measure INT8] ELF:     text={elf_size['text_bytes']:,}  "
    ...
    for map_candidate in [
        deeploy_root / "DeeployTest/TEST_GAP9/build_master/DSCNNL_INT8.map",
        deeploy_root / "DeeployTest/TEST_GAP9/build_master/memory_report_int8.txt",
    ]:
    ...
    # ── gapy command (same as cmake board_DSCNNL_INT8 target) ─────────────────
    ...
    print(f"\n[GAP9 Measure INT8] Starting {args.runs} inference runs...\n")
    ...
    for run_idx in range(1, args.runs + 1):
        ...
        cycles, errors_n, tested_n, pred_class, pred_logit = result
        ...
        logit_str = str(pred_logit) if pred_logit is not None else "?"

        print(f"{cycles:>12,} cy  {lat_ms:6.1f} ms  "
              f"err={err_str}  class={pred_class}({label})  logit={logit_str}")
    ...
    # ── Summary ────────────────────────────────────────────────────────────────
    n_ok = len(rows)
    print(f"\n[GAP9 Measure INT8] {n_ok}/{args.runs} runs succeeded  "
