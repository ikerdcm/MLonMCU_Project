...
uint8_t MicReadChunk(uint16_t *avg)
{
...
#ifdef ENABLE_MIC_PROCESSING
#ifdef ENABLE_MIC_DEBUG_STATUS
    static uint32_t empty_fifo_polls = 0;
#endif
#endif
    ...
    if (rx_size == 0) {
        empty_fifo_polls++;
        if ((empty_fifo_polls % 2500) == 0) {
            PR_INFO("WARN: I2S RX FIFO empty, no microphone samples captured yet\n");
        }
    } else {
        if (empty_fifo_polls >= 2500) {
            PR_INFO("I2S RX FIFO active again, rx_level=%lu\n", (unsigned long)rx_size);
        }
        empty_fifo_polls = 0;
    }
#endif
