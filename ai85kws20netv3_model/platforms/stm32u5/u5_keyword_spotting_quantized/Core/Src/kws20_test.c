...
AI_ALIGNED(32)
static kws20_input_elem_t ai_input_data[KWS_INPUT_SIZE];

AI_ALIGNED(32)
static kws20_output_elem_t ai_output_data[KWS_OUTPUT_SIZE];
...
static void fill_input_from(const float *src, float scale, int transpose)
{
    for (int i = 0; i < KWS_INPUT_SIZE; i++) {
        ...
        ai_input_data[i] = kws20_input_from_float(src[src_i] * scale);
    }
}
