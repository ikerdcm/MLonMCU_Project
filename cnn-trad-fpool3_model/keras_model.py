...
def _ds_cnn_body(inputs, filters, num_ds_blocks, weight_decay=1e-4):
  """Shared DS-CNN backbone: initial conv + N depthwise-separable blocks."""
...
def get_model(args):
  ...
  if model_name == "fc4":
    ...
    # MLCommons reference: 64 filters, 4 DS blocks — ~2.6M MACs (49x10 input)
    print("DS-CNN model invoked")
    filters = 64
    final_pool_size = (int(input_shape[0] / 2), int(input_shape[1] / 2))

    inputs = Input(shape=input_shape)
    x = _ds_cnn_body(inputs, filters=filters, num_ds_blocks=4)
    x = Dropout(rate=0.4)(x)
    x = AveragePooling2D(pool_size=final_pool_size)(x)
    x = Flatten()(x)
    outputs = Dense(label_count, activation='softmax')(x)
    model = Model(inputs=inputs, outputs=outputs)

  elif model_name == 'ds_cnn_l':
    # DS-CNN-L: 64 filters, 6 DS blocks — ~3.8M MACs (49x10 input)
    # Same width as kws_ref_model (ds_cnn) so the first 4 DS blocks can be
    # initialized from the reference checkpoint via finetune.py.
    print("DS-CNN-L model invoked")
    filters = 64
    final_pool_size = (int(input_shape[0] / 2), int(input_shape[1] / 2))

    inputs = Input(shape=input_shape)
    x = _ds_cnn_body(inputs, filters=filters, num_ds_blocks=6)
    x = Dropout(rate=0.4)(x)
    x = AveragePooling2D(pool_size=final_pool_size)(x)
    x = Flatten()(x)
    outputs = Dense(label_count, activation='softmax')(x)
