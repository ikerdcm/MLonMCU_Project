...
def draw_plot(highlight_mcu: str) -> None:
    ...
    # Gray dots first (behind)
    for mcu, label, acc, energy, flash, model, above in DATA:
        if mcu == highlight_mcu:
            continue
        ax.scatter(energy, acc, s=flash * BUBBLE, color=GRAY, alpha=0.50,
                   edgecolors=GRAY, linewidths=2.5, marker=MARKER[model], zorder=3)
        if above:
            ax.text(energy, acc + Y_OFF, label,
                    fontsize=56, color=GRAY, fontweight="bold",
    ...
    ax.set_title(f"Accuracy vs Inference Energy  —  {MCU_NAMES[highlight_mcu]} highlighted\n"
    ...
    color_h = [mpatches.Patch(color=COLORS[k] if k == highlight_mcu else GRAY,
                               label=n)
    ...
    out = OUT_DIR / f"plot_accuracy_vs_energy_{highlight_mcu}.png"
