...
OUT = ROOT / "final_results" / "max78000_ai85kws20netv3_model"
TABLES = OUT / "tables"
PLOTS = OUT / "plots"
...
KEY_METRICS = [
    ...
    ("mac_ops_per_cycle", "MAC/cycle", "MAC/cycle", "higher"),
...
def radar_plot(rows):
    ...
    cx, cy, r = 470, 390, 245
    body = title_block(
        "MAX78000 Online Best-Ratio Radar",
...
def bubble_frontier(rows, spec):
    ...
    if any(value is None for value in xs_raw + ys_raw + sizes_raw):
        body = title_block(
            spec["title"],
            spec["subtitle"],
        )
        body.append(text(
            width / 2,
            height / 2,
            "not available in the MAX78000 online summaries",
            "subtitle",
            "middle",
        ))
        body.append(text(
            width / 2,
            height / 2 + 24,
            f"missing: {x_key}, {y_key}, or {size_key}",
            "small",
            "middle",
        ))
...
def compute_delta_rows(rows):
    ...
    for key, label, unit, direction in KEY_METRICS:
        ...
        for name, variant, base, _ in COMPARISONS:
...
def improvement_heatmap(rows_delta):
    comparisons = [(f"{name}_improvement_pct", label) for name, _, _, label in COMPARISONS]
    values = [
        abs(row[key])
...
def improvement_profile_plot(rows_delta, metric_names, file_name, title, subtitle):
    ...
    if len(rows) < 2:
        body = title_block(title, subtitle)
        body.append(text(490, 280, "not enough comparable metrics for this profile", "subtitle", "middle"))
        save_svg(PLOTS / file_name, 980, 560, body)
        return
...
def power_window_overlay(reports, windows_by_variant, selection_by_variant, file_name, title, subtitle):
    ...
    for idx, value in enumerate(axis_ticks(xmin, xmax, 6)):
        ...
        body.append(line(x, top, x, bottom, "grid"))
        body.append(text(x, bottom + 22, fmt_ms_tick(value), "tick", "middle"))
...
def power_six_peak_grid(reports, windows_by_variant):
    ...
    for peak_number in range(1, 7):
        ...
        body.append(text(right, bottom + 17, fmt_ms_tick(x_max), "tick", "middle"))
...
def edge_panel(body, selected, x_transform, x_range, y_range, x0, y0, panel_w, panel_h, title, zero_label):
    ...
    for idx, value in enumerate(axis_ticks(xmin, xmax, 5)):
        x = left + idx * (right - left) / 4
        body.append(line(x, top, x, bottom, "grid"))
        body.append(text(x, bottom + 19, fmt_ms_tick(value), "tick", "middle"))
    for idx, value in enumerate(axis_ticks(ymin, ymax, 5)):
        y = bottom - idx * (bottom - top) / 4
        body.append(line(left, y, right, y, "grid"))
...
def power_edge_zoom_overlay(windows_by_variant, selection_by_variant, file_name, title, subtitle):
    ...
    if not selected:
        return
    max_duration = max(window["duration_ms"] for _, _, window in selected)
    pre_ms = max(1.5, min(55.0, max_duration * 1.4))
    post_ms = max(3.0, min(115.0, max_duration * 3.0))
    rising_range = (-pre_ms, post_ms)
    falling_range = (-post_ms, pre_ms)
    y_values = []
...
def power_analysis_plots_and_tables():
    ...
    windows_by_variant = {
        variant["id"]: read_power_trace_windows(reports[variant["id"]], pad_ms=5.0, max_points=900)
        for variant in VARIANTS
    }
...
def write_tables(rows, all_keys):
    ...
    md = [
        "| Metric | Unit | Direction | " + " | ".join(variant["label"] for variant in VARIANTS) + " |",
    ...
    tex = [
        "\\begin{tabular}{l" + "r" * (len(VARIANTS) + 1) + "}",
        "\\toprule",
        "Metric & Unit & " + " & ".join(variant["short"] for variant in VARIANTS) + " \\\\",
        "\\midrule",
    ]
    for key, label, unit, _ in KEY_METRICS:
        vals = [fmt_value(rows[v["id"]].get(key), unit) for v in VARIANTS]
        tex.append(f"{label} & {unit} & " + " & ".join(vals) + " \\\\")
    ...
    md = [
        "| Metric | Direction | " + " | ".join(label + " improvement" for _, _, _, label in COMPARISONS) + " |",
        "|---|---|" + "|".join("---:" for _ in COMPARISONS) + "|",
    ]
    for row in rows_delta:
        values = [pct(row.get(f"{name}_improvement_pct")) for name, _, _, _ in COMPARISONS]
        md.append(f"| {row['metric']} | {row['direction']} | " + " | ".join(values) + " |")
...
def write_dashboard():
    ...
    body = [
        ...
        "<title>MAX78000 ai85kws20netv3 Online Results</title>",
        "<style>",
...
def main():
    ...
    bar_grid(
        ...
        "Available MAX78000 model and compute values from the online summaries.",
        cols=3,
    )
    bar_grid(
        [
