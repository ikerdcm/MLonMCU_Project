...
# Edge-TPU v2 int8; receives raw audio from testbench.py).
# Usage:  ./scripts/build_and_flash_eval_stream.sh [path/to/coralmicro]
set -euo pipefail
...
if [[ ! -f "$BUILD_DIR/CMakeCache.txt" ]] || \
   ! grep -qF "CMAKE_HOME_DIRECTORY:INTERNAL=$CORALMICRO_ROOT" "$BUILD_DIR/CMakeCache.txt"; then
    ...
    # Light re-configure so a newly-added app (add_subdirectory) is picked up
    # (the cache check above won't trigger for a new target).
...
echo "Flash complete. Run: python3 ../testbench/testbench.py --board coral --model v1 --port <CDC>"
