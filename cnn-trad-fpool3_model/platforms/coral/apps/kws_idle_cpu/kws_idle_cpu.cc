...
// with the M7 in low-power sleep (WFI) between inferences. FP32 on the M7 CPU
// (config: fp32-cpu, NO Edge TPU). Purpose: power-meter measurement of the
// active-vs-idle duty cycle (active spike during Invoke, ~5 s WFI baseline).
//
...
//
// Model: /models/ds_cnn_l_float.tflite (float32 I/O). Offline input: the baked
//        "left" MFCC test vector (ds_cnn_test_input_left_float.h) — no mic.
...
constexpr int kTensorArenaSize = 2 * 1024 * 1024;   // FP32 activations, SDRAM
...
void run_idle_loop() {
    printf("\r\n==============================\r\n");
    printf("DS-CNN-L DUTY-CYCLE POWER (Coral M7 CPU, FP32, offline)\r\n");
    printf("==============================\r\n");
    ...
    printf("BENCH,event=model_info,mode=offline"
           ...
           ",version=ds_cnn_l_cpu_fp32_idle\r\n",
