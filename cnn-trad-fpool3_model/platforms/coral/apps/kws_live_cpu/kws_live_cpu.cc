// Live keyword spotting on Coral Dev Board Micro — FP32 on the M7 CPU.
// config: fp32-cpu (online). Captures mic audio at 16 kHz, computes MFCC, runs
// DS-CNN-L with plain TFLite-Micro (NO Edge TPU). Online counterpart of
// kws_bench_cpu; mirrors kws_live.cc (shared MFCC/VAD/serial protocol) so the
// dashboard parses it unchanged.
//
// Model: /models/ds_cnn_l_float.tflite (float32 I/O). The MFCC front-end still
// emits int8 features, so we dequantize them to float at the model input using
// the int8 model's input quant (scale/zero-point printed by
// scripts/convert_float_tflite.py).
...
// int8 MFCC → float model input. From ds_cnn_l_static.tflite input quant
// (see scripts/convert_float_tflite.py): f = (q - zero_point) * scale.
constexpr float kInScale     = 0.03684842586517334f;
constexpr int   kInZeroPoint = -9;

// FP32 activations are ~4x the int8 footprint; arena lives in SDRAM.
...
extern "C" void app_main(void* param) {
    ...
    printf("\r\n=== KWS Live — DS-CNN-L on M7 CPU (FP32) ===\r\n");
    ...
    // --- Interpreter (no Edge TPU: builtin float ops) ---
    ...
    while (true) {
        ...
        // Dequantize int8 MFCC → float model input.
        for (int i = 0; i < MFCC_OUTPUT_ELEMS; ++i)
            input->data.f[i] = (mfcc_buf[i] - kInZeroPoint) * kInScale;
        ...
        for (int i = 0; i < kNumClasses; ++i) {
            float score = output->data.f[i];  // float model: softmax prob
        ...
        // Decision view: per-class scores 0..100 for the dashboard bar chart.
        {
            ...
            for (int i = 0; i < kNumClasses; ++i) {
                int s = (int)(output->data.f[i] * 100.0f + 0.5f);
