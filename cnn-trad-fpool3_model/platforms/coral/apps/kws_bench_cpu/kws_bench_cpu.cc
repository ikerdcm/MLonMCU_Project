// DS-CNN-L offline benchmark — 50 back-to-back inferences on the M7 CPU.
// FP32 baseline (config: fp32-cpu): plain TFLite-Micro, NO Edge TPU op. This is
// the CPU side of the Coral TPU-vs-CPU curve (Story A FP32 row / Story B bottom
// rung). Mirrors kws_bench.cc so the emitted BENCH CSV is identical and the
// existing measurement tooling parses it unchanged.
//
// Model: /models/ds_cnn_l_float.tflite (float32 I/O), built by
//        scripts/convert_float_tflite.py from ds_cnn_l_finetuned.keras.
// Input: the "left" MFCC test vector, already dequantized to float in
//        ds_cnn_test_input_left_float.h (same physical input as the int8 bench).
...
#include "ds_cnn_test_input_left_float.h"
...
// FP32 activations are ~4x the int8 footprint; the arena lives in SDRAM so we
// can be generous. (int8 bench used 256 KiB.)
...
void run_bench() {
    printf("\r\n==============================\r\n");
    printf("DS-CNN-L BENCHMARK (Coral M7 CPU, FP32)\r\n");
    printf("==============================\r\n");
    ...
    // No Edge TPU: register the builtin float ops the DS-CNN-L graph uses.
    // (Model op set verified by convert_float_tflite.py: CONV_2D,
    //  DEPTHWISE_CONV_2D, AVERAGE_POOL_2D, FULLY_CONNECTED, SOFTMAX; RESHAPE
    //  registered as a harmless safety.)
    ...
    if (interpreter.AllocateTensors() != kTfLiteOk) {
        printf("ERROR: AllocateTensors() failed (arena too small?)\r\n");
        return;
    }
    ...
    if (input->bytes != (size_t)KWS_TEST_INPUT_FLOAT_SIZE) {
        printf("ERROR: input size mismatch (model=%u expected=%d)\r\n",
               (unsigned)input->bytes, KWS_TEST_INPUT_FLOAT_SIZE);
        return;
    }
    ...
    for (int run = 0; run < kBenchRuns; run++) {
        memcpy(input->data.f, kws_test_input_float, KWS_TEST_INPUT_FLOAT_SIZE);
        ...
        for (int i = 0; i < kNumClasses; i++) {
            float v = output->data.f[i];  // float model: output is softmax prob
